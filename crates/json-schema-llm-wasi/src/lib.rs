...
//!
//! WASI universal binary for json-schema-llm. Exposes `convert` and `rehydrate`
//! via a stable C ABI for consumption by any WASM runtime (wasmtime, wazero,
//! wasmer, etc.).
//!
//! ## Target
//!
//! Compile with: `cargo build --target wasm32-wasip1 --release -p json-schema-llm-wasi`
//!
//! ## ABI Contract
//!
...
pub extern "C" fn jsl_convert(
    ...
    let default_opts = serde_json::to_string(&json_schema_llm_core::ConvertOptions::default())
    ...
    result_from_bridge(json_schema_llm_core::convert_json(
        &schema_str,
        &effective_opts,
    )) as u32
}
...
pub extern "C" fn jsl_rehydrate(
    ...
    result_from_bridge(json_schema_llm_core::rehydrate_json(
        &data_str,
        &codec_str,
        &schema_str,
    )) as u32
...
pub extern "C" fn jsl_list_components(schema_ptr: u32, schema_len: u32) -> u32 {
    ...
    result_from_bridge(json_schema_llm_core::list_components_json(&schema_str)) as u32
}
...
pub extern "C" fn jsl_extract_component(
    ...
    let default_extract_opts =
        serde_json::to_string(&json_schema_llm_core::ExtractOptions::default())
    ...
    result_from_bridge(json_schema_llm_core::extract_component_json(
        &schema_str,
        &pointer_str,
        &effective_opts,
    )) as u32
...
pub extern "C" fn jsl_convert_all_components(
    ...
    let default_conv_opts = serde_json::to_string(&json_schema_llm_core::ConvertOptions::default())
    ...
    let default_ext_opts = serde_json::to_string(&json_schema_llm_core::ExtractOptions::default())
        .expect("default extract options serialize");
    ...
    result_from_bridge(json_schema_llm_core::convert_all_components_json(
        &schema_str,
        &effective_conv_opts,
        &effective_ext_opts,
    )) as u32
