...
use json_schema_llm_core::{convert_json, rehydrate_json, ConvertError, ErrorCode};
...
fn test_rehydrate_json_valid() {
    ...
    let codec = r#"{"$schema": "https://json-schema-llm.dev/codec/v1", "transforms": [], "droppedConstraints": []}"#;
...
fn test_api_version_stability() {
    use json_schema_llm_core::API_VERSION;
...
fn test_codec_camel_case_serialization() {
    use json_schema_llm_core::Codec;
    let codec = Codec::new();
...
fn test_warning_camel_case_serialization() {
    ...
    use json_schema_llm_core::Warning;
...
fn test_codec_version_mismatch() {
    ...
    let codec = r#"{"$schema": "https://json-schema-llm.dev/codec/v99", "transforms": [], "droppedConstraints": []}"#;
...
fn test_convert_result_deserialize_roundtrip() {
    use json_schema_llm_core::{convert, ConvertOptions, ConvertResult};
